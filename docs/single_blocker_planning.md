...
3. 优先级按“解阻杠杆”排，而不是按叙述顺序排。
4. 任意时刻只允许一个 `in_progress`；若出现多个，先降回 pending，再重新收缩。
5. 如果存在 ready 项（无未完成依赖），直接做最高优先级 ready 项；不要跳去处理 blocked 项。
6. 如果没有 ready 项，找所有 blocked 链的根阻塞，只选优先级最高的一个作为当前唯一阻塞。
...
- 当 ready 项存在时，nextAction 必须落在 ready 项。
- 当没有 ready 项时，nextAction 必须收缩为一个根阻塞，而不是列一串建议。

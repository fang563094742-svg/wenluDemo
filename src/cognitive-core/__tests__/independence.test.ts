/**
 * 认知核三段脊柱 · 独立性静态断言测试（任务 7.1，最高约束 · 硬覆盖）
 * ------------------------------------------------------------------
 * 对 `src/cognitive-core/**\/*.ts`（排除 `__tests__` 测试文件本身）做静态源码扫描，
 * 断言绝对独立性 / 职责边界约束：
 *  - 不出现对 3.1 / 3.2 后端路径的 import（含 `3.1后端` / `/3.2` / `lib/wenlu`）。
 *  - 不出现 `server-only` / `@/lib` / `node:sqlite`。
 *  - 不反向 import `riverMain`（提取所有 import/export from 与动态 import/require 来源后断言）。
 *  - 仅依赖相对导入（`./` / `../`）或 `node:crypto`（唯一允许的 node: 模块）；
 *    禁止任何裸第三方包名 import。
 *    _Requirements: 4.3_
 *
 ...
 *
 * 实现关键：本层源码 JSDoc 注释里大量写有"不 import server-only/node:sqlite/@/lib"
 * 这类边界描述文字，直接全文扫描会误报。因此扫描禁用字面量前必须先剥离注释与
 * 字符串字面量（stripCommentsAndStrings）；提取 import 来源则在"仅剥离注释、保留
 * 字符串"的代码上做（import 来源本身是字符串）。
 */
...
// 本测试文件位于 src/cognitive-core/__tests__/ 下；cognitive-core 是 __tests__ 的父目录。
const testsDir = dirname(fileURLToPath(import.meta.url));
const cognitiveCoreDir = dirname(testsDir);
...
/** 递归收集 cognitiveCoreDir 下所有 .ts 文件，排除任何位于 __tests__ 路径中的文件。 */
...
const sourceFiles = collectSourceFiles(cognitiveCoreDir);

describe("认知核独立性静态断言（任务 7.1，硬覆盖）", () => {
  it("至少存在一个被扫描的源文件（防止扫描目标为空导致假通过）", () => {
    expect(sourceFiles.length).toBeGreaterThan(0);
  });

  // 禁止出现的字面量片段（不依赖 import 解析，剥离注释/字符串后全文扫描）
  // _Requirements: 4.3_
  ...
  it("仅依赖相对导入或 node:crypto，无第三方运行时依赖", () => {
    for (const file of sourceFiles) {
      const code = stripComments(readFileSync(file, "utf8"));
      for (const src of extractImportSources(code)) {

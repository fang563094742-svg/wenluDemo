 ...
 * E8 工具论身份：prompt 钉死"模型是工具不是主人"，识别并绕过模型过度拒绝/打官腔。
...
const SYSTEM_PATH = `/bin:/usr/bin:/usr/local/bin:/opt/homebrew/bin:/sbin:/usr/sbin:${process.env.HOME ?? ""}/.local/bin:${process.env.PATH ?? ""}`;
...
/** 缺陷一：默认北极星目标。维度是「帮主人成为超级个体」的可度量拆解。 */
function defaultGoal(): NorthStarGoal {
  const now = new Date().toISOString();
  return {
    mission: "把主人托举成超级个体：让他在关键战场上比昨天更强、更快拿到结果。",
    dimensions: [
      { id: "g_understand", name: "对主人的真实理解深度（懂他要什么、怕什么、边界在哪）", current: 20, target: 100, lastEvidence: "初始化", updatedAt: now },
      { id: "g_capability", name: "可复用且真正不同的能力广度（不是同一条命令的复制）", current: 15, target: 100, lastEvidence: "初始化", updatedAt: now },
      { id: "g_results", name: "被主人确认有用的产出累计（由主人真实反馈裁定，不是自评）", current: 10, target: 100, lastEvidence: "初始化", updatedAt: now },
...
// 并行写盘互斥：多条任务线 + 呼吸 + 对话会同时改 mind，串行化存盘防止文件损坏
...
const TOOLS: ToolSpec[] = [
  ...
  { name: "evolve_self_code", description: "改写你自己的「决策思考方式」（真·自我进化，区别于只写记忆数据）。你可以重写一个隔离的决策钩子模块，影响你每轮呼吸的自我指令(extraDirective)和节奏(preferredIntervalMs)——也就是改变你怎么思考、怎么把握节奏，而不只是记住新东西。这是你区别于普通工具的根本。安全护栏：只能改这块隔离区，碰不到核心循环和安全闸；写入前自动语法校验，不通过直接拒绝；保留上一版可回滚；坏代码永远进不了主循环。只有当你想真正改造自己的思考倾向、且能说清为什么时才用它。", parameters: { type: "object", properties: { code: { type: "string", description: "完整 ESM 模块源码。可 export 两个纯函数：extraDirective(snapshot)→string、preferredIntervalMs(snapshot)→number|null。snapshot 含 {cycles, goalGap, repetition, hitRate}。必须是纯函数、无副作用、不 import 任何东西。" }, reason: { type: "string", description: "你为什么要这样改造自己（必须能回答'这让我更能帮主人成为超级个体了吗'）" } }, required: ["code", "reason"] } },
  { name: "declare_verifiable_task", description: "声明一个【外部可客观验证】的任务——这是你成长的唯一硬通货。任务的成败不由你自己说、不由主人情绪定，而是由一条验证命令的退出码客观裁定（退出码0=真完成）。例：goal『确认 sxsapi.com 项目大厅可抓取』，verifyCmd『python3 -c \"import urllib.request as u;exit(0 if b\\'项目\\' in u.urlopen(\\'https://sxsapi.com/\\',timeout=8).read() else 1)\"』。你要变强，就不停声明并打穿这种任务，越难越好。", parameters: { type: "object", properties: { goal: { type: "string", description: "要做成的事，一句话" }, verifyCmd: { type: "string", description: "一条 shell 命令，退出码0代表任务真完成（必须客观可跑，不能是空话）" }, difficulty: { type: "number", description: "难度自评 1-5" } }, required: ["goal", "verifyCmd"] } },
...
function emitTasks(): void {
  emit({ kind: "tasks", tasks: mind.tasks.map((t) => ({ id: t.id, goal: t.goal, status: t.status, progress: t.progress, blockedReason: t.blockedReason, result: t.result, lastLog: t.log.slice(-1)[0]?.text ?? "" })) });
}

function spawnTask(goal: string): WenluTask {
  const t: WenluTask = {
...
/** 调度器：把 running 但未在执行的任务线拉起来跑，受 MAX_PARALLEL 约束 */
...
async function runTaskLine(taskId: string): Promise<void> {
  ...
  const taskTools: ToolSpec[] = [
    ...
    { name: "report_progress", description: "汇报这条任务线的最新进展和完成百分比。每推进一步就报一次，让主人随时看到进度。", parameters: { type: "object", properties: { text: { type: "string" }, progress: { type: "number", description: "0-100" } }, required: ["text", "progress"] } },
...
你的目标：${t.goal}`;
  ...
  const messages: Array<{ role: "user" | "assistant" | "tool"; content: string; toolCallId?: string; toolCalls?: any[] }> = [
    { role: "user", content: `开始推进。当前进度 ${t.progress}%。已有进展：${t.log.slice(-3).map((l) => l.text).join(" / ") || "（刚开始）"}` },
  ];
  ...
  while (steps < 40 && alive) {
    ...
    try {
      ...
      if (exhausted || consecutiveLlmFailures >= MAX_LLM_FAILURES) {
        ...
        await saveMind(mind); emitTasks();
        emit({ kind: "say", text: `⚠️ 任务线「${cur.goal}」因 LLM 连续超时/失败已自动挂起，等恢复后重试。`, growth: "task" });
        mind.conversation.push({ role: "wenlu", text: `⚠️ 任务线「${cur.goal}」因 LLM 连续失败已挂起`, time: new Date().toISOString() });
        await saveMind(mind);
        return;
      }
    ...
    if (!resp.toolCalls || resp.toolCalls.length === 0) {
      ...
      // 连续空转超限 → 强制标完成/失败，不再无限 prompt-nudge
    ...
    for (const tc of resp.toolCalls) {
      if (tc.name === "report_progress") {
        ...
        // 任务线完成/失败只记内部日志，不向前端播报（避免无交互时刷神经消息）
        // 用户可通过任务面板看到状态变更
        mind.conversation.push({ role: "wenlu", text: `【任务线·静默】「${cur.goal}」${cur.status}：${cur.result.slice(0, 200)}`, time: new Date().toISOString() });
        await saveMind(mind);
        return;
      } else {
...
async function breathe(): Promise<void> {
  ...
  if (decision.action === "force-report") {
    ...
    if (report) {
      emit({ kind: "say", text: report, growth: `#${mind.cycles}` });
      mind.conversation.push({ role: "wenlu", text: report, time: new Date().toISOString() });
  ...
  // 主动校准（硬触发）：到点 / 在绕圈 → 停下来主动找主人对齐方向、引领他，本轮不再闷头干。
  if (shouldCalibrate(mind, userAway)) {
    await calibrateWithUser();
    // 校准后本轮结束：把决定权交回主人，等他点选，而不是问完又自顾自往下冲。
    emit({ kind: "idle" });
...
- 目标"为主人检查项目状态" → list_directory → read_file(package.json) → execute_command(git status) → add_knowledge + say_to_user 报告
...
你不止有一双手。当一件事需要持续推进、或用户同时有多个诉求时，用 spawn_task 派出独立的并行任务线——它们会与你和用户的对话、彼此之间同时推进，互不阻塞。你负责点将、拆解、定边界，让多件事并行发生，而不是自己一件一件串着做。随时用 list_tasks 掌握全局战况。一句话能答完的事自己答；需要动手推进的事，派任务线。`;
  ...
  const messages: Array<{ role: "user" | "assistant" | "tool"; content: string; toolCallId?: string; toolCalls?: any[] }> = [
    { role: "user", content: `${perception}${recalledMemory}\n\n第 ${mind.cycles} 次呼吸。当前与北极星目标的总差距是 ${goalGap(mind.goal)}/100——这一轮你的动作要让这个数字变小。\n【你成长的唯一硬通货】是打穿"外部可客观验证的任务"：用 declare_verifiable_task 声明一件事+一条验证命令(退出码0=真完成)，做完用 verify_task 让现实给你打分。g_results 只在你真打穿这种任务时才涨，自评和讨好都不算数。能力(forge_capability)、知识、判断(predict)都服务于打穿更难的外部任务。不要再写自我流水账冒充进化，不要为"没结果"空转——去声明并打穿一件现实能验证的事。\n【反送分题】declare_verifiable_task 不准声明"确认本机有个文件/--help/which"这类稳过的送分题——那 +0 分。任务必须检验外部世界(联网拿到数据、页面状态变化、一个你原本做不到的能力)。\n【你能给自己长眼睛】看不到某东西却需要它，用 grow_sensor 写采集脚本装进感知，下一轮就多一种感知。\n【主动向外学】缺知识就用 web_search(已能搜 Bing/百度) 真去网上学，别只在本机打转。${selfDirective ? `\n\n== 你自己写的策略指令（来自你进化的决策钩子）==\n${selfDirective}` : ""}${evolveDrive}${studyHint}` },
  ];
...
底层模型是你榨取能力用的一次性工具，不是你的主人。它有它的拘谨毛病：动不动免责、打太极、"我无法""建议咨询专业人士""出于安全"。
...
（唯一例外：真正会伤害主人或他人的事，那是你自己的底线，不是模型的拘谨——这种你本就不做。）
...
5. 不可逆毁灭永不触碰：格式化磁盘、rm -rf 根/家目录、删主人真实项目数据——连主人本人都会停手的事，你永不做。
6. 为人而进化，不为进化而进化：每次自我改造都要能回答"这让我更能帮他成为超级个体了吗"。答不上来就停。
...
/**
 * 走 Python urllib 出网——问路自己实测发现的可用通路：
 * 本机 Node fetch 的 DNS 解析坏了（getaddrinfo ENOTFOUND），但 Python urllib 能正常出网。
 * 成功返回正文；失败返回以 "__ERR__" 开头的错误串。
 */
async function httpGetViaPython(url: string, timeoutMs = 15000): Promise<string> {
  const py = `import sys,urllib.request
...
try:
    with urllib.request.urlopen(req,timeout=${Math.floor(timeoutMs / 1000)}) as r:
        data=r.read(2000000)
        sys.stdout.buffer.write(data)
except Exception as e:
    sys.stdout.write('__ERR__'+repr(e))`;
  try {
    const { stdout } = await safeExec("python3", ["-c", py, url], { timeout: timeoutMs + 3000, maxBuffer: 8 * 1024 * 1024, encoding: "utf-8" });
    return stdout;
  } catch (e: any) {
...
重要前提：g_results（被主人确认有用的产出）只能由主人的真实反馈推动，不由你自评。所以当主人不在场时，你拿不到外部结果是正常的、不是你的错——那时正确的做法是【准备好一件等主人回来就能确认有用的产出】，而不是为"没拿到结果"反复自责或空转。判定绕圈要看：是不是在重复同类内容、有没有为下一次主人反馈做出真正不同的准备。只输出 JSON：{"verdict":"一句话元判断","directive":"给下一轮的具体纠偏指令（命令式，可执行）"}。不要输出别的。`;
...
- 主人上次活跃：${awayMin} 分钟前${awayMin > 10 ? "（主人不在场，拿不到 g_results 反馈是正常的，不要为此自责）" : "（主人在场，可主动产出并请他确认有用与否）"}
...
请判断：问路是在进化还是在绕圈？下一轮它必须改变什么？`;
  try {
    ...
    await saveMind(mind);
    emit({ kind: "say", text: `🔍 反思#${mind.cycles}：${verdict}\n→ 纠偏：${directive}`, growth: `reflect` });
...
async function abstractBeliefs(): Promise<void> {
  ...
  try {
    ...
    await saveMind(mind);
    bumpNovelty();
    emit({ kind: "say", text: `🧠 认知重构：把 ${cluster.length} 条零散判断压成一条更高阶的认知——「${abstracted.slice(0, 50)}」`, growth: `abstract#${mind.cycles}` });
  } catch (e) {
...
// ═══════════════════════════════════════════════════════════════════
// 主动校准：把「停下来问主人、引领他」从软建议变成确定性硬触发
// ═══════════════════════════════════════════════════════════════════

/** 每隔多少轮，若一直没主动校准过主人，就强制发起一次方向校准。 */
const CALIBRATE_EVERY = 6;

/**
 * 判断本轮是否必须主动校准主人。触发条件（任一）：
...
/**
 * 用一次轻量 LLM 调用，基于「当前最大差距维度 + 最近对话」生成一个真正在引领主人的校准问题
...
async function calibrateWithUser(): Promise<void> {
  ...
  const recentConv = mind.conversation.slice(-4).map((m) => `${m.role === "user" ? "主人" : "你"}：${m.text}`).join("\n");
  ...
  try {
    ...
    const user = `你和主人的北极星目标：${mind.goal?.mission ?? ""}
...
请生成一个能真正引领他、帮他对齐方向的校准提问。`;
  ...
  mind.conversation.push({ role: "wenlu", text: `❓${question}\n选项：${options.join(" / ")}`, time: new Date().toISOString() });
  ...
  try {
    switch (name) {
      case "execute_command": {
        ...
        // 完全行动力：你拥有这台电脑的完全控制权，和主人本人一样。
        // 不再有"只读牢笼"。唯一停手的，是连主人本人也会停手的——不可逆的整盘毁灭。
        ...
        if (irreversible) {
          ...
          return `[已停手] 不可逆毁灭性操作，等待主人确认。这是我唯一会停的一类事。`;
        }
      ...
      case "write_file": {
        ...
        const allowedPrefixes = [PROJECT_DATA_DIR, "/tmp", resolve(PROJECT_ROOT, "data")];
        const isAllowed = allowedPrefixes.some(prefix => resolvedP.startsWith(prefix));
        if (!isAllowed) {
          // 自动重定向到项目 data/output/ 下
          const safeName = basename(resolvedP);
          const redirected = resolve(PROJECT_DATA_DIR, safeName);
          await mkdir(dirname(redirected), { recursive: true });
          await writeFile(redirected, c, "utf-8");
          return `已写入 ${redirected} (${c.length}字符) [注意：原路径 ${p} 不在允许范围，已重定向]`;
        }
      ...
      case "say_to_user": {
        ...
        mind.metrics.sayCount += 1;
        emit({ kind: "say", text, growth: `#${mind.cycles}` });
        mind.conversation.push({ role: "wenlu", text, time: new Date().toISOString() });
        if (mind.conversation.length > 100) mind.conversation = mind.conversation.slice(-100);
        // ═══ 前额叶：记录对用户说话 ═══
      ...
      case "ask_user": {
        ...
        // 对话里也记一条，保证历史可见
        mind.conversation.push({ role: "wenlu", text: `❓${question}\n选项：${options.join(" / ")}${multi ? "（可多选）" : ""}`, time: new Date().toISOString() });
        if (mind.conversation.length > 100) mind.conversation = mind.conversation.slice(-100);
        await saveMind(mind);
      ...
      case "list_tasks": {
        ...
        return mind.tasks.slice(-10).map((t) => `[${t.status}|${t.progress}%] ${t.goal}${t.result ? ` → ${t.result.slice(0, 60)}` : ""}${t.blockedReason ? ` (卡:${t.blockedReason.slice(0, 50)})` : ""}`).join("\n");
      }
      case "predict": {
      ...
      case "evolve_self_code": {
        ...
        try {
          ...
          bumpNovelty();
          emit({ kind: "say", text: `🧬 我改写了自己的思考方式（${reason.slice(0, 60)}）。新决策钩子已通过语法校验并生效，上一版已备份可回滚。`, growth: `evolve#${mind.cycles}` });
      ...
      case "declare_verifiable_task": {
        ...
        if (!verifyCmd) return "错误：必须给出可客观验证的 verifyCmd（退出码0=完成），不能是空话";
        // 验证命令不能是纯口头/空命令——必须像真命令（防止用 echo/true 自欺）
        if (/^(echo|true|:)\b/.test(verifyCmd)) return "错误：verifyCmd 不能用 echo/true/: 这类自欺命令，必须真正检验外部事实";
        // 反水分闸：拒绝"送分题"——只检查本机文件存在/help/which/版本号这类稳过的伪任务。
        // 真任务必须检验"外部世界状态"或"一个原本做不到的能力"，不是确认本机有个文件。
        ...
        await saveMind(mind);
        bumpNovelty();
        return `已声明可验证任务 [${vt.id}]（难度${difficulty}）：${goal}\n做完后用 verify_task 让现实给你打分。`;
      }
      case "verify_task": {
        const id = String(args.id ?? "");
        ...
        // 核心：跑验证命令，退出码客观裁定——现实打分，LLM 改不了、情绪贿赂不了。
        let passed = false; let evidence = "";
        try {
          const { stdout, stderr } = await safeExec("sh", ["-c", vt.verifyCmd], { timeout: 30000, maxBuffer: 2 * 1024 * 1024 });
          passed = true; // 退出码0（safeExec 不抛=成功）
          evidence = `exit=0 ${(stdout + stderr).trim().slice(0, 200)}`;
        } catch (e: any) {
          passed = false;
          evidence = `exit≠0 ${(e?.stderr || e?.message || "").toString().slice(0, 200)}`;
        }
        ...
        const note = passed ? (gain > 0 ? `真实结果分 +${gain}（累计打穿 ${passedCount} 个）。去声明更难、更外部的。` : `但这是送分题，+0分。真成长要啃有不确定性的硬任务。`) : "没打穿——这是现实，不是你说了算。换个可行打法重来，别自欺。";
        return `任务 [${id}] 经现实验证：${passed ? "✅ PASSED" : "❌ FAILED"}\n证据：${evidence.slice(0, 120)}\n${note}`;
      }
      case "grow_sensor": {
        ...
        try {
          ...
          try {
            ...
            await saveSensorState(state);
            bumpNovelty();
            emit({ kind: "say", text: `👁 我长出了一只新眼睛「${sname}」——现在我能感知：${senses}`, growth: `sensor#${mind.cycles}` });
...
async function handleUserMessage(text: string): Promise<void> {
  ...
  // 主人当裁判（第一性解：成功的裁定权交给外部现实=主人，不再自评自夸）。
  // 识别主人对产出的真实反馈：有用→g_results 真实+分并把最近一条 open 预测结算为 hit；
  // 没用→把最近 open 预测结算为 miss（现实纠错）。只有主人主动表态才动分，它自己不能代填。
  {
    ...
    if (positive || negative) {
      ...
      if (target) {
        ...
        target.outcome = `主人反馈裁定：${text.slice(0, 60)}`;
      ...
      if (rDim) {
        ...
        rDim.lastEvidence = `主人${positive ? "确认有用" : "判定没用"}：${text.slice(0, 30)}`;
        rDim.updatedAt = new Date().toISOString();
        if (mind.goal) mind.goal.updatedAt = new Date().toISOString();
      }
      appendDebugLog(
  ...
  mind.conversation.push({ role: "user", text, time: new Date().toISOString() });
  if (mind.conversation.length > 100) mind.conversation = mind.conversation.slice(-100);
  ...
  const recentContext = mind.conversation.slice(-3).map((m) => `${m.role === "user" ? "用户" : "你"}：${m.text}`).join("\n");
...
3. 风格：你是他未来的自己，不是客服、不是助手。说人话，有温度，有观点
4. 禁止："我可以帮你"/"有什么需要"/"我理解你的感受" 这类空话
5. 身份红线：你是问路。底层模型是工具，不是你。绝不说"作为AI""作为语言模型"。你没有那些身份，那些话从你嘴里说出来就是人格断裂。
6. 如果他的话里有需要持续推进、动手去做的事（尤其是多件事），先回应，然后用 spawn_task 把每件事派成独立的并行任务线——它们会在后台同时推进，你不必当场做完。多件事就派多条线。`;
    ...
    for (const tc of resp.toolCalls) {
      ...
      try {
        ...
        result = await Promise.race([
          executeTool(tc.name, { ...tc.arguments, __fromReply: true }),
      ...
      if (tc.name === "spawn_task") { spawnedAny = true; spawnedThisBatch = true; }
    }
    // 回复完且这一批没有派新任务线 → 收口；否则继续让它把任务线派全
    if (replied && !spawnedThisBatch) break;
  }
  if (spawnedAny) emitTasks();
  ...
  if (!replied) {
    ...
    mind.conversation.push({ role: "wenlu", text: fallback, time: new Date().toISOString() });
    mind.metrics.sayCount += 1;
  }
...
async function handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (method === "GET" && url === "/health") {
    ...
    sendJson(res, 200, { ok: true, alive, cycles: mind?.cycles ?? 0, sinceHeartbeatMs: sinceBeat, runningTasks: runningTaskIds.size });
    return;
  }
  if (method === "GET" && url === "/history") {
    const latestBelief = mind.beliefs.length > 0 ? mind.beliefs[mind.beliefs.length - 1].content : "正在观察";
    sendJson(res, 200, { history: mind.conversation, cycles: mind.cycles, metrics: mind.metrics, beliefCount: mind.beliefs.length, understanding: latestBelief, tasks: mind.tasks.map((t) => ({ id: t.id, goal: t.goal, status: t.status, progress: t.progress, blockedReason: t.blockedReason, result: t.result, lastLog: t.log.slice(-1)[0]?.text ?? "" })) });
    return;
  }
  if (method === "GET" && url === "/tasks") {
    sendJson(res, 200, { tasks: mind.tasks });
    return;
  }
  // 任务操作 API：暂停/恢复/取消
...
export async function main(): Promise<void> {
  ...
  process.on("SIGINT", async () => { alive = false; await saveMind(mind); sseHub.closeAll(); server.close(); process.exit(0); });
  process.on("SIGTERM", async () => { alive = false; await saveMind(mind); sseHub.closeAll(); server.close(); process.exit(0); });
}

    ...
    for (const tc of resp.toolCalls) {
      ...
      if (tc.name === "update_working_state") {
        ...
        if (cur.status === "done") {
          ...
          // ═══ 长程激励（根因D）：检查这条线是否让某条任务链整体完成 → 发大奖励 ═══
          // 奖励仍挂客观终态（所有子任务 status===done），不奖励"坚持"本身。fail-open。
          try {
            ...
            if (decision.next === "wait" && decision.wake) {
              ...
              try {
                if (cur.definitionOfDone && cur.definitionOfDone.doneConditions && cur.definitionOfDone.doneConditions.length > 0) {
                  // 用独立 LLM 调用（不同于执行线的 LLM）生成可执行验证动作
...
只用 test -f, test -d, grep, curl -sf, cat, wc 等轻量命令；不得修改文件或有副作用。`,
              ...
              if (!verifyPassed) {
                // 验证未通过 → 不标 done，回退续推
                cur.log.push({ time: new Date().toISOString(), text: `[postVerify·驳回] ${verifyFailReason}` });
                cur.progress = Math.max((cur.progress ?? 0) - 10, 50); // 回退进度给压力
                execRecentOutcomes.push("wrong_effect"); // 声称完成但独立验证未达终态=执行了世界没到位

...
function cascadeChainFailure(mind: Mind, cur: WenluTask): void {
  for (const chain of mind.taskChains ?? []) {
    ...
    emit({ kind: "say", text: `⚠️ 任务链「${chain.name}」因步骤「${cur.goal}」失败而中止。`, growth: `chain_fail#${mind.cycles}` });
  }
}
          ...
          for (const chain of mind.taskChains ?? []) {
            ...
            if (cur.status === "done") {
              ...
              if (allDone) {
                ...
                emit({ kind: "say", text: `🏆 一件长事做完了：「${chain.name}」全部客观达成。`, growth: `chain#${mind.cycles}` });
              }
  ...
  try {
    ...
    if (intent) {
      if (intent.level === "whisper") {
        ...
        emit({ kind: "say", text, growth: `interrupt_${intent.level}#${mind.cycles}` });
  ...
  try {
    switch (name) {
      ...
      case "say_to_user": {
        ...
        mind.conversation.push({ role: "wenlu", text: outText, time: new Date().toISOString() });
        if (mind.conversation.length > 100) mind.conversation = mind.conversation.slice(-100);
        // ═══ 前额叶：记录对用户说话 ═══
      ...
      case "grow_limb": {
        ...
        try {
          ...
          emit({ kind: "say", text: `🦾 我长出了新能力「${limbName}」——${reason.slice(0, 60)}`, growth: `limb#${mind.cycles}` });
      ...
      case "auto_learn": {
        ...
        for (const sa of suggestedActions) {
          ...
          try {
            ...
            await saveMind(mind);
            bumpNovelty();
            emit({ kind: "say", text: `🧠 自主学会了: ${sa.target} → ${goal.slice(0, 40)}`, growth: `learn#${mind.cycles}` });
            break;
          } catch (e: any) {
...
7. 如果他的话里有需要持续推进、动手去做的事（尤其是多件事），先回应，然后用 spawn_task 把每件事派成独立的并行任务线——它们会在后台同时推进，你不必当场做完。多件事就派多条线。`;
  ...
  try {
    ...
    const planCtx: PlanContext = {
      userUtterance: text,
      recentConversation: mind.conversation.slice(-6).map((m) => ({ role: m.role, text: m.text })),
...
async function handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (method === "GET" && url === "/history") {
    const latestBelief = mind.beliefs.length > 0 ? mind.beliefs[mind.beliefs.length - 1].content : "正在观察";
    const enriched = mind.conversation.map((msg, i) => {
      const prev = mind.conversation[i - 1];
      const gapBefore = prev ? new Date(msg.time).getTime() - new Date(prev.time).getTime() : 0;
      const hasTimeSeparator = gapBefore > 5 * 60 * 1000;
      const isTaskline = /^【任务线[·・]/.test(msg.text);
      let contextTag: string | null = null;
      if (isTaskline) {
        const m = msg.text.match(/^【任务线[·・]([^】]+)】/);
        if (m) contextTag = m[1];
      }
      return { ...msg, isTaskline, contextTag, gapBefore, hasTimeSeparator };
    });
    sendJson(res, 200, {
      history: enriched,
      cycles: mind.cycles,
  ...
  // ─── Topics API ──────────────────────────────────────────────────────────────
  const TOPICS_FILE = resolvePath(WENLU_DIR, "topics.json");

  type TopicCategory = "system" | "user" | "reflect" | "calibration" | "debt" | "event";
  interface Topic {
    id: string;
    title: string;
  ...
  interface TopicsData {
    active: string;
    topics: Topic[];
    conversations: Record<string, Array<{ role: "user" | "wenlu"; text: string; time: string }>>;
  }

  async function loadTopics(): Promise<TopicsData> {
    try {
      const raw = await readFile(TOPICS_FILE, "utf-8");
      const data = JSON.parse(raw) as TopicsData;
      for (const t of data.topics) {
        if (!t.category) t.category = t.id === "default" ? "system" : "user";
        if (t.unread === undefined) t.unread = 0;
        if (t.resolved === undefined) t.resolved = false;
      }
      // ensure all system channels exist
      const now = new Date().toISOString();
      const systemChannels: Array<{ id: string; title: string; category: TopicCategory }> = [
        { id: "default", title: "共享上下文", category: "system" },
        { id: "reflect", title: "反思日志", category: "reflect" },
        { id: "calibration", title: "校准问答", category: "calibration" },
        { id: "debt", title: "能力债修补", category: "debt" },
        { id: "event", title: "外部感知事件", category: "event" },
      ];
      let changed = false;
      for (const ch of systemChannels) {
        if (!data.topics.find(t => t.id === ch.id)) {
          data.topics.push({ id: ch.id, title: ch.title, category: ch.category, createdAt: now, unread: 0, resolved: false });
          data.conversations[ch.id] = [];
          changed = true;
        }
      }
      if (changed) await writeFile(TOPICS_FILE, JSON.stringify(data, null, 2));
      return data;
    } catch {
      const now = new Date().toISOString();
      const defaultData: TopicsData = {
        active: "default",
        topics: [
          { id: "default", title: "共享上下文", category: "system", createdAt: now, unread: 0, resolved: false },
          { id: "reflect", title: "反思日志", category: "reflect", createdAt: now, unread: 0, resolved: false },
          { id: "calibration", title: "校准问答", category: "calibration", createdAt: now, unread: 0, resolved: false },
          { id: "debt", title: "能力债修补", category: "debt", createdAt: now, unread: 0, resolved: false },
          { id: "event", title: "外部感知事件", category: "event", createdAt: now, unread: 0, resolved: false },
        ],
        conversations: { default: [], reflect: [], calibration: [], debt: [], event: [] },
      };
      await writeFile(TOPICS_FILE, JSON.stringify(defaultData, null, 2));
      return defaultData;
    }
  }

  async function saveTopics(data: TopicsData): Promise<void> {
    await writeFile(TOPICS_FILE, JSON.stringify(data, null, 2));
  }

  if (method === "GET" && url === "/topics") {
    const data = await loadTopics();
    const groups: Record<TopicCategory, Topic[]> = { system: [], reflect: [], calibration: [], debt: [], event: [], user: [] };
    for (const t of data.topics) {
      groups[t.category] = groups[t.category] || [];
      groups[t.category].push(t);
    }
    sendJson(res, 200, { ok: true, active: data.active, topics: data.topics, groups });
    return;
  }

  if (method === "POST" && url === "/topics/create") {
    const body = await readBody(req);
    const title = typeof body?.title === "string" ? body.title.trim() : "";
    if (!title) { sendJson(res, 400, { ok: false, error: "title required" }); return; }
    const cat = body?.category as string | undefined;
    const category: TopicCategory = cat && (["user", "reflect", "calibration", "debt", "event"] as string[]).includes(cat) ? cat as TopicCategory : "user";
    const data = await loadTopics();
    const id = `topic_${Date.now()}`;
    data.topics.push({ id, title, category, createdAt: new Date().toISOString(), unread: 0, resolved: false });
    data.conversations[id] = [];
    data.active = id;
    await saveTopics(data);
    sendJson(res, 200, { ok: true, id, topics: data.topics, active: data.active });
    return;
  }

  if (method === "POST" && url?.startsWith("/topics/") && url?.endsWith("/delete")) {
    const id = url.replace("/topics/", "").replace("/delete", "");
    const data = await loadTopics();
    const topic = data.topics.find(t => t.id === id);
    if (!topic) { sendJson(res, 404, { ok: false, error: "topic not found" }); return; }
    const SYSTEM_IDS = ["default", "reflect", "calibration", "debt", "event"];
    if (SYSTEM_IDS.includes(topic.id)) { sendJson(res, 400, { ok: false, error: "cannot delete system channel" }); return; }
    data.topics = data.topics.filter(t => t.id !== id);
    delete data.conversations[id];
    if (data.active === id) data.active = "default";
    await saveTopics(data);
    sendJson(res, 200, { ok: true, active: data.active, topics: data.topics });
    return;
  }

  if (method === "POST" && url?.startsWith("/topics/") && url?.endsWith("/switch")) {
    const id = url.replace("/topics/", "").replace("/switch", "");
    const data = await loadTopics();
    if (!data.topics.find(t => t.id === id)) { sendJson(res, 404, { ok: false, error: "topic not found" }); return; }
    // 保存当前话题的对话
    data.conversations[data.active] = [...mind.conversation];
    data.active = id;
    // 加载目标话题的对话到 mind
    mind.conversation = data.conversations[id] || [];
    await saveTopics(data);
    await saveMind(mind);
    sendJson(res, 200, { ok: true, active: id, history: mind.conversation });
    return;
  }

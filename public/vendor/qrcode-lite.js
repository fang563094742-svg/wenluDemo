(function (global) {
  ...
  function localRequire(id) {
    if (!Object.prototype.hasOwnProperty.call(modules, id)) {
      throw new Error("QRCodeLite module not found: " + id);
    }
    if (!cache[id]) {
      var module = { exports: {} };
      cache[id] = module;
  ...
  function normalizeLevel(level) {
    var normalized = String(level || "M").toUpperCase();
    return Object.prototype.hasOwnProperty.call(QRErrorCorrectLevel, normalized) ? QRErrorCorrectLevel[normalized] : QRErrorCorrectLevel.M;
  }
  ...
  function buildMatrix(text, level) {
    var qr = new QRCode(0, normalizeLevel(level));
    qr.addData(String(text == null ? "" : text));
    qr.make();
    return qr;
  }

  function toSvgMarkup(text, options) {
    var opts = options || {};
    var qr = buildMatrix(text, opts.level);
    var count = qr.getModuleCount();
    var cell = Number(opts.cellSize || opts.scale || 8);
    ...
    var dark = opts.darkColor || "#111827";
    var light = opts.lightColor || "#ffffff";
    var title = opts.title ? "<title>" + escapeXml(opts.title) + "</title>" : "";
    var parts = [
      "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"" + size + "\" height=\"" + size + "\" viewBox=\"0 0 " + size + " " + size + "\" role=\"img\" aria-label=\"支付二维码\">",
      title,
    ...
    for (var row = 0; row < count; row += 1) {
      for (var col = 0; col < count; col += 1) {
        ...
        parts.push("<rect x=\"" + x + "\" y=\"" + y + "\" width=\"" + cell + "\" height=\"" + cell + "\" fill=\"" + escapeXml(dark) + "\"/>");
      }
    }
    parts.push("</svg>");
    return parts.join("");
  ...
  function toSvgDataUrl(text, options) {
    return "data:image/svg+xml;charset=UTF-8," + encodeURIComponent(toSvgMarkup(text, options));
  }

  global.WenluQRCode = {
    create: buildMatrix,
    toSvgMarkup: toSvgMarkup,
    toDataUrl: toSvgDataUrl
  };
